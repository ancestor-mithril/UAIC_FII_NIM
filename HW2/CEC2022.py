...
def test(func_name, vector, expected):
    vector = eval(vector)
    vector = [[x] for x in vector]
    x = np.array(vector)

    func_num = func_dict[func_name]
    func = cec2022_func(func_num)
    func.values(x)

    print(func_name, f"Got {func.ObjFunc}, expected {expected}, diff = ", np.fabs(func.ObjFunc - expected))


if __name__ == "__main__":
    func = "cf04"
    vector = "[-71.182594,-99.997977,-99.997861,-79.726332,95.075462,-95.867247,95.709858,64.054248,28.473938,99.997915]"
    expected = 165.445
    test(func, vector, expected)
    
    func = "cf03"
    vector = "[66.327940,36.349401,-24.861653,15.460897,79.992593,68.979176,21.222719,8.560249,-4.474606,-81.094566]"
    expected = 150.488
    test(func, vector, expected)
    
    func = "cf02"
    vector = "[-31.676608,-2.474135,38.632281,94.728926,15.692491,-74.323191,6.026589,53.172961,-14.786363,39.071642]"
    expected = 217.35
    test(func, vector, expected)
    
    func = "cf01"
    vector = "[-99.999756,-99.999104,-99.839844,100.000000,-64.047605,-84.782930,74.498202,43.713400,4.021990,53.735744]"
    expected = 229.302
    test(func, vector, expected)
    
    func = "hf03"
    vector = "[-15.240903,73.944035,64.035480,44.431434,61.671899,-24.039429,84.649811,2.853773,49.314342,33.314684]"
    expected = 6.18957
    test(func, vector, expected)
    
    func = "hf02"
    vector = "[-37.731754,-76.870181,81.441440,-52.187828,69.964666,-50.004605,78.967726,14.183194,-23.060768,-62.331476]"
    expected = 20
    test(func, vector, expected)
    
    func = "hf01"
    vector = "[16.006914,5.779696,25.380196,-48.829887,12.237741,-45.473867,-63.704986,-67.403249,-60.863711,-80.063547]"
    expected = 0.279
    test(func, vector, expected)
    
    func = "levy_func"
    vector = "[-24.189693,-1.571958,24.609199,54.102313,-22.873517,-4.937637,15.888914,-1.903588,-54.722542,40.666694]"
    expected = 7.62399
    test(func, vector, expected)
    
    func = "rastrigin_func"
    vector = "[48.154840,-57.400609,14.767369,-17.050190,20.864126,-47.916270,32.828069,-38.742814,-0.882110,4.313772]"
    expected = 19.8991
    test(func, vector, expected)
    
    func = "schaffer_F7_func"
    vector = "[79.089393,-24.572778,-34.673926,-44.681621,75.997880,16.363316,-21.742985,5.658653,64.985498,51.267961]"
    expected = 7.07548e-09
    test(func, vector, expected)
    
    func = "rosenbrock_func"
    vector = "[87.495032,16.961076,-68.407979,64.912626,-39.771721,-24.163209,22.806024,62.104952,-59.450873,53.289258]"
    expected = 3.98658
    test(func, vector, expected)
    
    func = "zakharov_func"
    vector = "[-55.938324,4.543096,35.300113,8.279385,-47.436938,7.302106,6.387677,-2.497854,-61.053554,-47.134047]"
    expected = 9.9521e-09
    test(func, vector, expected)
