                        ...
                        for (auto ct : crossoverTypes) {
                            for (auto hc : hillclimbings) {
                                for (auto hs : hyperMutationSteps) {
                                    ...
                                    encodingChanges.begin(), encodingChanges.end(),[&](auto ec) {
                                        auto copy = i++;
                                         std::cout << copy << '\n';
                                        std::ofstream f{"experiments/" + std::to_string(copy) +
                                                        ".txt"};
                                        f << "Crossover : " << c << '\n';
                                        f << "Mutation : " << m << '\n';
                                        ...
                                        for (auto exp = 0; exp < 3; ++exp) {
                                            ga::GeneticAlgorithm ga{
                                                c,  m,    m * 10,      e,    s,
                                                ct, hc,   100,    10,   hs,
                                                ec, 2000, "rastrigin_func", true, true};
                                            auto rez = ga.run();
                                            f << rez << '\n';
                                        }
                                    });
